...
var itemBarInventory: Array[Resource] = [
	...
	preload("uid://bdk58u1bflwxl"), null, null, null, null
]
...
func use_item(usedItem: ITEM_BAR_ITEM, value):
	for i in itemBarInventory.size():
		var inventoryItem = itemBarInventory[i]
		
		if not inventoryItem:
			continue
		
		if inventoryItem.itemID == usedItem.itemID:
			inventoryItem.value -= value
			
			if inventoryItem.value == 0:
				itemBarInventory[i] = null
				currentItem = null
