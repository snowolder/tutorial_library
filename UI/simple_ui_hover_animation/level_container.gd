...
func _on_mouse_entered() -> void:
	...
	tween.tween_property(self, "offset_transform_rotation",deg_to_rad(10), 0.1)
	tween.parallel()
	tween.tween_property(self, "offset_transform_position",Vector2(0,-10), 0.1)
	
func _on_mouse_exited() -> void:
	add_theme_stylebox_override("panel", styleBoxUnselected)
	var tween := create_tween()
	tween.tween_property(self, "offset_transform_rotation",deg_to_rad(0), 0.1)
	tween.parallel()
