...
func _on_slow_motion_area_body_entered(body: Node2D) -> void:
	if "slowMotionMultiplier" in body:
		body.slowMotionMultiplier = 0.1

func _on_slow_motion_area_body_exited(body: Node2D) -> void:
	if "slowMotionMultiplier" in body:
		body.slowMotionMultiplier = 1.0
