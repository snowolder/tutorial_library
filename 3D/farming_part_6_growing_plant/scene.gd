...
func _load_farm():
	...
	for groundData in farmData["ground"]:
		...
		for objectKeyWord in groundData["objectKeyWord"]:
			...
			var isPlant = "Plant" in objectName
			...
			if isSoil:
				...
				var plantData = GameManager.get_data("plant", objectId)
				var plantResource = load(plantData.plantResourcePath)
				var plantNode = sceneGround.add_plant(plantResource)
				plantNode.load_data(plantData)
