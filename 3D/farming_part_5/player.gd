...
func _input(_event: InputEvent) -> void:
	...
	elif Input.is_action_just_pressed("actionE"):
		var currentItem = item_bar.get_current_item()
		
		if currentItem:
			_item_interaction(currentItem)
...
func _item_interaction(item: ITEM_BAR_ITEM):
	if lastRaycastCollision.has_method("item_interaction") and item:
		_do_interaction_animation()
		
		var numberUsedItems = lastRaycastCollision.item_interaction(item)
		if numberUsedItems:
			item_bar.use_item(item, numberUsedItems)

func _do_interaction_animation():
	onAction = true
	set_physics_process(false)
